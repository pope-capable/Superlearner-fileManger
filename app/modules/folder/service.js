...
export async function list({id:userId},{pageId,limit}) {
  try {
    const totalCount = await Folder.count({where:{userId}});
    const {offset,pageCount} = paginate(totalCount,pageId);
    const folders = await Folder.findAll({where:{userId}, offset, limit:parseInt(limit)});
    return {
      success : true,
      data : {
        folders,
