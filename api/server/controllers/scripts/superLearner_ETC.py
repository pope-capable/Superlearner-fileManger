...
try:
    ...
    filename = Output_name+'_SL_RF_finalized_model.mod'
    with open( filename, "wb") as file:
    ...
    s3_resource.meta.client.upload_file(
        Filename=Output_name+'_SL_RF_finalized_model.mod',Bucket='superlearner',Key=Output_name+'_SL_RF_finalized_model.mod')
    s3_resource.meta.client.upload_file(
        Filename=Output_name+'test.txt',Bucket='superlearner',Key=Output_name+'test.txt')
    
    key= Output_name+'_Modelfile.mod'
    key_two = Output_name+'_SL_RF_finalized_model.mod'
    ...
    payload = { "projectId": projectID,"processId": c,"location1": New_url, "location2":New_url_two, "location3":New_url_three, "name3":key_three, "type3":"txt", "name": Output_name}
    headers = {'token': token}
    ...
    payload = {"processId": c, "projectId":projectID, "name":key, "reason":ex}
    headers = {'token': token}
